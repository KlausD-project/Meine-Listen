...
const STORAGE = {
  todo: "todo_mobile_v1",
  shop: "shop_mobile_v1",
  birthday: "birthday_mobile_v1",
  events: "events_mobile_v1"
};
...
function updateDateTime() {
  ...
  if (node) node.innerHTML = `${dd}.${mm}.${yyyy}<br>${time} Uhr`;
}
updateDateTime();
...
function saveItems(type, items) {
  localStorage.setItem(STORAGE[type], JSON.stringify(items));
}
...
function shareList(type) {
  const items = loadItems(type);
  if (!items.length) { alert("Keine Einträge zum Teilen."); return; }
  let text = "";
  if (type === "birthday") {
    items.forEach(i => {
      const age = i.birthDate ? calcAge(i.birthDate) : "";
      text += `${i.text}${age ? " — " + age + " Jahre" : ""}\n`;
    });
  } else {
    items.forEach(i => text += `- ${i.text}\n`);
  }
  if (navigator.share) {
    navigator.share({ title: `Meine ${type} Liste`, text }).catch(()=>{});
  } else {
...
/* ---------- Render Lists + Drag & Drop for todo & birthday ---------- */
function renderList(type) {
  ...
  items.forEach((item, idx) => {
    ...
    const draggable = (type === "todo" || type === "birthday");
    ...
    cb.addEventListener("change", () => {
      ...
      const localIdx = arr.findIndex(i => i.id === item.id);
      if (localIdx >= 0) {
        arr[localIdx].done = !arr[localIdx].done;
        saveItems(type, arr);
        renderList(type);
      }
    });
    ...
    if(type === "birthday" && item.birthDate){
      ...
      span.textContent = `${item.text}${age ? " — " + age + " Jahre" : ""}`;
    } else {
    ...
    meta.className = "meta";
    if(type === "birthday" && item.birthDate){
      meta.textContent = item.birthDate;
    } else {
...
/* ---------- Initial Render ---------- */
setupUI();
sections.forEach(s=>renderList(s));
